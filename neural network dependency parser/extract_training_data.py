...
class FeatureExtractor(object):
    ...
    def __init__(self, word_vocab_file, pos_vocab_file):
        self.word_vocab = self.read_vocab(word_vocab_file)    # word-to-index dictionary     
        self.pos_vocab = self.read_vocab(pos_vocab_file)      #  POS-to-index dictionary 
    ...
    '''
    input 
        a list of words in the input sentence
        a list of POS tags in the input sentence
        an instance of class State. 
    output
    example
        buffer: "dog(top) eats a" 
        stack : "the"(top), <ROOT> 
        output: ind for( "the", <ROOT>, <NULL>, "dog", "eats", "a")
        <CD> stands for any number
        <NNP> stands for any proper name
        <UNK> stands for unknown words, appears only once
        <ROOT> is a special root symbol, word 0
    ...
    def get_input_representation(self, words, pos, state):
        ...
        res = []
        # stack
        len_stk = len(state.stack)
        for i in range(len_stk - 1, len_stk - min(len_stk, 3) - 1, -1):
            key = ""
            if state.stack[i] == 0: 
                key = '<ROOT>'
                ...
                key = '<CD>'
            elif words[state.stack[i]].lower() not in self.word_vocab:
                key = '<UNK>'
            else:
                key = words[state.stack[i]].lower()
            res.append(self.word_vocab[key])
        for i in range(0, 3 - len_stk):
            res.append(self.word_vocab['<NULL>'])

        # buffer
        len_buf = len(state.buffer)
        for i in range(len_buf - 1, len_buf - min(len_buf, 3) - 1, -1):
            key = ""
            if state.buffer[i] == 0: 
                key = '<ROOT>'
                ...
                key = '<CD>'
            elif words[state.buffer[i]].lower() not in self.word_vocab:
                key = '<UNK>'
            else:
                key = words[state.buffer[i]].lower()
            res.append(self.word_vocab[key])
        for i in range(0, 3 - len_buf):
            res.append(self.word_vocab['<NULL>'])

        return np.array(res)

    '''
    input : (transition, label) pair
    output: one-hot representation of length 91
    function: tf.keras.utils.to_categorical(y, num_classes=None, dtype="float32")
    '''
